...
export function EditContractPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['contract', id], queryFn: () => getContract(id!), enabled: !!id })
  ...
  useEffect(() => {
    ...
    setRetentionPercent(String(data.retentionPercent))
    setStatus(data.status)
    setStartDate(parseDisplayDate(data.startDate))
    setEndDate(parseDisplayDate(data.endDate))
    setNotes(data.notes ?? '')
  }, [data])
  ...
  return (
    <div className={styles.page}>
      <nav className={styles.breadcrumb}>Home &gt; Contracts &gt; Edit</nav>
      <h1>Edit Contract</h1>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.draft} onClick={() => navigate(`/contracts/${id}`)}>Cancel</button>
