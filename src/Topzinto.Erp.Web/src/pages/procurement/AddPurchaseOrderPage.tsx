...
export function AddPurchaseOrderPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (po) => navigate(`/procurement/${po.id}`),
  })
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!poNumber.trim() || !title.trim() || !supplierId || !lines.some((l) => l.description.trim())) return
    mutation.mutate()
  }

  return (
    <div className={styles.page} style={{ maxWidth: 960 }}>
      <nav className={styles.breadcrumb}>Home &gt; Procurement &gt; New PO</nav>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        <div className={styles.row}>
          <label>
            PO Number *
            <input value={poNumber} onChange={(e) => setPoNumber(e.target.value)} placeholder="PO-2024-004" required />
          </label>
          <label>
            Status
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.draft} onClick={() => navigate('/procurement')}>Cancel</button>
