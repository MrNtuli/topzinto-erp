...
[Authorize(Policy = ErpModules.TendersPolicy)]
public class TendersController : ControllerBase
{
    private readonly ITenderService _service;

    public TendersController(ITenderService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateTenderRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateTenderRequest request, CancellationToken ct)
    {
