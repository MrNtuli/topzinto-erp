...
[Authorize(Policy = ErpModules.SafetyPolicy)]
public class SafetyController : ControllerBase
{
    private readonly ISafetyService _service;

    public SafetyController(ISafetyService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateSafetyIncidentRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateSafetyIncidentRequest request, CancellationToken ct)
    {
